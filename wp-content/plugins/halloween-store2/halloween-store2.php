...
// создаем подпункт меню Halloween Masks в настройках
    ...
    <div class="wrap">
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr valign="top">
                    <th scope="row">
                    <?php _e('Show Product Inventory', 'halloween-plugin') ?></th>
                    <td>
                        <input type="checkbox"
                               name="halloween_options[show_inventory]"
                            <?php echo checked($hs_inventory, 'on'); ?> />
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row">
                    <?php _e('Currency Sign', 'halloween-plugin') ?></th>
                    <td><input type="text"
...
// отображаем форму метаполя
    echo "
        <table>
            <tr>
                <td>
                    " .__('Sku', 'halloween-plugin')." :
                    </td>
                <td>
                    <input type='text' name='halloween_product_sku' value= '".esc_attr( $hween_sku )." ' size='10'>
                </td>
            </tr>
            <tr>
                <td>
                    ". __('Price', 'halloween-plugin')." :
                </td>
                <td>
                    <input type='text' name='halloween_product_price' value='".esc_attr( $hween_price )." ' size='5'>
                </td>
            </tr>
            <tr>
                <td>
                    " .__('Weight', 'halloween-plugin')." :
                </td>
                <td>
                    <input type='text' name='halloween_product_weight' value='".esc_attr( $hween_weight )." ' size='5'>
                </td>
            </tr>
            <tr>
                <td>
                " .__('Color', 'halloween-plugin')." :
                </td>
                <td>
                    <input type='text' name='halloween_product_color' value='".esc_attr( $hween_color )." ' size='5'>
                </td>
                </tr>
                <tr>
                    <td>Inventory:</td>
                    <td>
                        <select name='halloween_product_inventory' id='halloween_product_inventory'>
                            <option value='In Stock ".selected( $hween_inventory, 'In Stock', false ). " '>
                                " .__( 'In Stock', 'halloween-plugin' ). "
                            </option>
                            <option value='Backordered ".selected( $hween_inventory, 'Backordered', false ). " '>
                                " .__( 'Backordered', 'halloween-plugin' ). "
                            </option>
                            <option value='Out of Stock ".selected( $hween_inventory, 'Out of Stock', false ). " '>
                                " .__( 'Out of Stock', 'halloween-plugin' )."
                            </option>
                            <option value='Discontinued ".selected( $hween_inventory, 'Discontinued', false ). " '>
                ...
                <!--shortcode-->
                    <td colspan='2'>
                        <strong>
                            " .__( 'Shortcode Legend', 'halloween-plugin' )."
                        </strong>
                    </td>
                </tr>
                <tr>
...
class hs_widget extends WP_Widget
{
    ...
    function form($instance)
    {
        ...
        <p><?php _e('Number of Products', 'halloween-plugin') ?>:
            <input
                ...
                type="text"
                value="<?php echo esc_attr($number_products); ?>"
                size="2"
                maxlength="2"
            />
        </p>
    ...
    function update( $new_instance, $old_instance )
    {
        ...
        $instance['number_produts'] = absint( $new_instance['number_products'] );
        return $instance;
    }
...
// произвольный запрос, чтобы извлечь продукты
        ...
        while ($dispProducts->have_posts()) : $dispProducts->the_post();
            ...
            echo '<p>' . __('Price', 'halloween-plugin') . ': '
                . $hween_options_arr['currency_sign'] . $hs_price .
                '</p>';
