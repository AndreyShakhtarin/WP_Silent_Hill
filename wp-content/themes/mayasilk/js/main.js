...
(function($) {
	...
	function initNavigation() {
		...
		// Nav Widget
		function wersenaNavWidget() {
			if (getWindowWidth() <= 991 || $('body').hasClass('mobile')) {
				$(document).on('click', '.search-right > a', function(e) {
					if($(this).parent('.search-right').hasClass('open')) {
						$('.search-right').removeClass('open');
					} else {
						$('.search-right').removeClass('open');		
						$(this).parent('.search-right').addClass('open');
					}
					e.stopPropagation();
					e.preventDefault();
				});
			} else {
				$(document).on('click', '.search-right[data-trigger="click"] > a', function(e) {
					if($(this).parent('.search-right').hasClass('open')) {
						$('.search-right').removeClass('open');
					} else {
						$('.search-right').removeClass('open');		
		...
		function initDesktopNav() {
			...
			var navHoverElems = [$('.search-right[data-trigger="hover"]')];
			var navHoverResult = $();
...
jQuery(document).ready(function () {
	   jQuery('nav#mayasilk-main').meanmenu();
});
        ...
        var timer_metaslider_8 = function() {
            var slider = !window.jQuery ? window.setTimeout(timer_metaslider_8, 100) : !jQuery.isReady ? window.setTimeout(timer_metaslider_8, 1) : custom_slider(window.jQuery);
        };
        timer_metaslider_8();
/*!
